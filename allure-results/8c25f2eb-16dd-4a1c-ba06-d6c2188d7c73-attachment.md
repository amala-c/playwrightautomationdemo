...
- Location: tests/logout.spec.js:11:5

# Error details

```
TypeError: loginObj.loggingOut is not a function
```
    ...
    - list [ref=e6]:
      ...
      - listitem [ref=e16]:
        - link "Log out" [ref=e17] [cursor=pointer]:
          - /url: "#"
      - listitem [ref=e18]:
        - link "Welcome amala" [ref=e19] [cursor=pointer]:
          - /url: "#"
      - listitem
    - generic [ref=e21]:
      - list [ref=e22]:
...
```ts
  ...
  4  | 
  5  | const loginData=require('../utils/login.json')
  6  | 
  7  | test.beforeEach('loginurl',async({page})=>{
  8  |     await page.goto("https://demoblaze.com/")
  9  | })
  10 | //Test Case - 01 - Login with valid user //
  11 | test('logout valid user',async({page})=>{
  12 | 
  13 |     const login_username=loginData.lusername
  14 |     const login_password=loginData.lpassword
  15 |     const loginObj=new LoginNewUser(page)
  16 |     
  17 |     const title =await page.title()
  18 |     //console.log("Page title is "+title)
  ...
  23 | 
  24 |     const logoutObj=new UserLogout(page)
> 25 |     await loginObj.loggingOut()
     |                    ^ TypeError: loginObj.loggingOut is not a function
  26 | })
```